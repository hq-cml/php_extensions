...
dnl PHP_ARG_ENABLE(hello_world, whether to enable hello_world support,
dnl Make sure that the comment is aligned:
dnl [  --enable-hello_world           Enable hello_world support])
...
if test "$PHP_HELLO_WORLD" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $HELLO_WORLD_DIR/lib, HELLO_WORLD_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_HELLO_WORLDLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong hello_world lib version or lib not found])
  dnl ],[
  dnl   -L$HELLO_WORLD_DIR/lib -lm
  dnl ])
  dnl
